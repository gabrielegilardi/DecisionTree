...
class RTLearner:
    ...
    def buildTree(self, X, Y):
        ...
        column 2 = relative position left branch (0 indicates no left branch -> leaf)
        column 3 = relative position right branch (0 indicates no right branch -> leaf)
        """
        # Stop and return the mean if equal or less than the lowest number of leaves
        if (X.shape[0] <= self.leaf):
            return np.array([-1, Y.mean(), 0, 0])

        # Stop and return the mean if all remaining leaves have close values
        Ym = Y.mean()
        ...
        # Keep splitting
            ...
            # Stop and return the mean if there is no split (because all data end up in
            # the left branch).
