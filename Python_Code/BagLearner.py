...
class BagLearner():
    ...
    def createModel(self, X, Y):
        ...
        prob = np.ones(n) / n           # Initial probability distribution (uniform)
        pred_Y = np.zeros(n)            # Initialize running sum of the predictions

        # Build the model for each learner
        for i in range(self.bags):

            # Randomly pick (with replacement) 'm' rows from the training data using the 
            # probability distribution 'prob' (always uniform distribution if no boosting)
            # and train the learner with the corresponding bag of data
            idx = np.random.choice(a, size=m, replace=True, p=prob)
            self.ensemble[i].createModel(X[idx,:], Y[idx])

            # If boosting is requested the probabilities are proportional to the absolute
            # value between actual label and the current prediction's average
            if (self.boost):

                # Add the prediction of the current learner to the previous predictions
